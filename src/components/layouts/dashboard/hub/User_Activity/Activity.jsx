...
import { getUsers, updateAttendance } from '../../../../../redux/async/userAsync';
...
const Activity = () => {
    ...
    const onSelect = (id, value) => {
        ...
        setAttendance([...filteredAttend, { id, attendance: { date, reason: value } }])
    }
    ...
    let rows = useMemo(() => {
        return users.map((el) => ({ ...el, status: ['present', 'absent', 'excused absent'] }))
    }, [users])
                ...
                <div className={style.board}>
                    ...
                    <div className={style.body}>
                        ...
                        <div className={style.batch}>
                            <Select label={"Choose a batch"} placeholder={"choose"} theme={"dark"} data={["1", "2", "3", "4", "5", "6"]} onChange={(value) => setBatch(value)} />
                        </div>

                    </div>
                </div>
                <div className={style.user_data}>
                    {users ? <Table elements={rows} titles={columns} theme={"dark"} select={true} onSelect={onSelect} /> :
                        <>
                    ...
                    <Pagination
                        total={totalUsers}
                        onChange={(i) => dispatch(getUsers({ page: i - 1, limit: 5, batch }))}
                        position="center"
                        styles={(theme) => ({
                            item: {
                                '&[data-active]': {
